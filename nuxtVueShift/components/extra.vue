...
async function getProject () {
    ...
    try{
        const { data, pending, refresh, error } = await useFetch('/api/extra/', {
            method: 'GET',
...
async function deleteData(row) {
    try {
        ...
        const { data, pending, refresh, error } = await useFetch(`/api/extra/${row.id}`, {
        method: 'DELETE',
        baseURL: 'http://localhost:8000',
        headers: {
            Authorization: `JWT ${useStore.token}`,
...
async function updateData() {
    ...
    try {
        const { data, pending, refresh, error } = await useFetch(`/api/extra/${editProject.id}/`, {
            method: 'PATCH',
            baseURL: 'http://localhost:8000',
            headers: {
                Authorization: `JWT ${useStore.token}`,
...
<template>
    ...
    <n-drawer v-model:show="activeDrawerProjectEdit" :width="502" :placement="placementDrawer">
        <n-drawer-content title="超排項目名稱" closable>
            <n-form>
                <n-form-item-row label="超排項目代碼">
                    <n-input placeholder="..." v-model:value="editProject.name" />
                </n-form-item-row>
                <n-form-item-row label="描述">
                    <n-input placeholder="詳細超排項目描述" v-model:value="editProject.description" />
                </n-form-item-row>
    ...
    <n-drawer v-model:show="activeDrawerProjectAdd" :width="502" :placement="placementDrawer">
        <n-drawer-content title="新增超排項目" closable>
            <n-form>
                <n-form-item-row label="超排項目代碼">
                    <n-input placeholder="..." v-model:value="newProject.name" />
                </n-form-item-row>
                <n-form-item-row label="描述">
                    <n-input placeholder="詳細超排項目描述" v-model:value="newProject.description" />
                </n-form-item-row>
