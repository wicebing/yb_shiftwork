...
class Table_groups(models.Model):
    id = models.AutoField(primary_key=True,verbose_name='ID')
    groupname = models.ForeignKey(Table_groupname,on_delete=models.CASCADE,verbose_name='群組名稱')
...
class Table_Shift_Schedule(models.Model):
    id = models.AutoField(primary_key=True,verbose_name='ID')
    date = models.ForeignKey(Table_date,on_delete=models.CASCADE,verbose_name='日期')
    shift = models.ForeignKey(Table_shift,on_delete=models.CASCADE,verbose_name='班別')
    project = models.ForeignKey(Table_project,on_delete=models.CASCADE,verbose_name='專案')
    staff = models.ForeignKey(Table_staff,on_delete=models.CASCADE,verbose_name='人員',null=True)
    def __str__(self):
...
class Table_meeting(models.Model):
    ...
    staff = models.ForeignKey(Table_staff,on_delete=models.CASCADE,verbose_name='人員')
    def __str__(self):
        return str(self.id)+self.date.date+self.project.name+self.staff.name
